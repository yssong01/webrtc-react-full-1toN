# 📡 WebRTC 1:N 실시간 화상·채팅·보드·화면공유 시스템

**React + Node.js + WebRTC + Socket.IO** 기반으로 구현한 1:N 실시간 커뮤니케이션 서비스입니다.  
영상/음성/화면공유/화이트보드/채팅/메모 기능을 모두 포함하며, WebRTC의 구조와 실전 적용을 학습하기 위한 프로젝트입니다.
...
WebRTC(Web Real-Time Communication) 기술을 활용하여 브라우저만으로 실시간 화상 회의, 화면 공유, 협업 도구를 구현한 교육용 프로젝트입니다. 플러그인 없이 순수 웹 기술로 다자간 통신을 구현하며, WebRTC의 핵심 개념과 실전 활용법을 학습합니다.

### 핵심 특징

- ✅ **플러그인 불필요**: 브라우저만으로 실시간 통신
- ✅ **1:N 화상 회의**: 여러 참가자 동시 연결
- ✅ **P2P 암호화**: 종단 간 보안 통신
- ✅ **실시간 협업**: 화면 공유, 화이트보드, 메모 공유
- ✅ **유연한 네트워크**: STUN/TURN 서버로 다양한 네트워크 환경 지원
...
**WebRTC**(Web Real-Time Communication)는 브라우저와 모바일 앱 간에 **실시간 오디오·비디오·데이터**를 P2P 방식으로 주고받을 수 있게 해주는 오픈 표준 기술입니다.
...
> 💡 **중요**: 이 프로젝트에서 Signaling 서버는 **Node.js + Socket.IO**로 구현되어, 브라우저 간의 WebRTC 관련 제어 메시지를 전달합니다.
...
- 브라우저 **F5 새로고침** 시 WebRTC 연결 완전 초기화
- 다시 **[방 입장] → [화상 시작]**으로 재참여
- 테스트 및 디버깅 용이
...
![WebRTC 1:N 실시간 통신 데모](./WebRTC-1toN.gif)

> 💡 **GIF 파일이 보이지 않나요?** 
> - GitHub 저장소에 `1toN.gif` 파일이 업로드되어 있는지 확인하세요
> - 또는 [여기](./1toN.avi)에서 원본 영상을 다운로드하세요

### 시연 주요 장면

| 기능 | 설명 |
|------|------|
| 🚪 **입장 흐름** | 방 번호, 이름 입력 → 공통 방 `room-1` 입장 |
| 🎥 **화상 시작** | 카메라/마이크 권한 요청 → 영상 타일 표시 |
| 👥 **참가자 추가** | 새 유저 입장 시 자동으로 타일 추가, 6명 이상 시 스크롤 |
| 🖥️ **화면 공유** | 어떤 참가자든 화면 공유 가능, 전체 브로드캐스트 |
| ✏️ **보드 필기** | 실시간 펜/지우개 기능, 모든 브라우저에 동기화 |
| 💬 **채팅** | 입장/퇴장 시스템 메시지 + 일반 채팅, 색상/크기 조절 |
| 🎤 **마이크** | 발화 시 영상 테두리 연두색 강조, 레벨바 표시 |
| 📝 **공유 메모** | 리치 텍스트 실시간 편집 (B/I/U/S, 색상, 크기) |

### 원본 영상 파일

- **파일명**: `1toN.avi` (고화질)
- **파일명**: `1toN.gif` (README 임베드용)
- **내용**: 1:N 화상 / 화면공유 / 보드 / 채팅 / 메모 기능 전체 데모
...
**팀 프로젝트 과제**: React와 Node.js를 결합한 실시간 웹 서비스 WebRTC 구현하기
...
WebRTC 기술 학습과 실전 구현을 통해 실시간 통신의 원리와 P2P 네트워크의 복잡성을 이해할 수 있었습니다. 이 프로젝트가 WebRTC를 학습하는 분들께 도움이 되기를 바랍니다.
